...
class TestCosmosClient:
    ...
    async def test_initialize_creates_db_and_containers(
        ...
        with patch("agent_stack.database.client.AzureCosmosClient") as mock_azure_cls:
            ...
            assert (
                mock_db.create_container_if_not_exists.call_count
                == _EXPECTED_CONTAINER_COUNT
            )
    ...
    async def test_database_property_returns_after_init(
        self, client: CosmosClient
    ) -> None:
